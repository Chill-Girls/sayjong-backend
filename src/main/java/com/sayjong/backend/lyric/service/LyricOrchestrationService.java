...
public class LyricOrchestrationService {
	...
	public boolean createAndSaveLyrics(AppLyricRequest request) {
		...
		if (lyricLineRepository.existsBySong_SongId(songId)) {
			log.warn("[LyricOrchestration] Skipped: Lyrics for songId {} already exist.", songId);
			return false;
		}

		log.debug("[LyricOrchestration] Finding song info for songId: {}", songId);
		...
		log.info("[LyricOrchestration] Requesting lyric creation from worker for: {}", nodeRequestPayload);
		...
		if (result == null) {
			log.error("[LyricOrchestration] Failed to get valid result from worker for songId: {}", songId);
			throw new IllegalStateException("Failed to get valid result from worker.");
		}

		lyricCreationService.saveLyrics(result, song);

		log.info("[LyricOrchestration] Process completed for songId: {}", songId);
		return true;
	}
}
